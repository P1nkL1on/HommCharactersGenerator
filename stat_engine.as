class stat_engine{
	...
	static function test(){
		...
		A(o, 'рана', new Array(2, 4, 8), 'состо€ние-болезнь', undefined, en);
		A(o, 'рана', new Array(3, 1, 16), 'состо€ние-болезнь', undefined, statWithName(o, 'рана', 0));
	...
	static function addStat(who, stat:Object){
		...
		if (statExistWithName(who, stat.name)){
			// т.е. если это обычный стат, то низ€
		...
		if (stat.name != undefined){
			...
			if (isEmpty){
				ut.TraceDebug('+ ƒобавлено "' + stat.name + '", но, тк значение не передано, ему будет присвоено дефолтное значение.');
	...
	static function watchUniqueStatValueParam(who, statName:String, paramIndex):Number{
		...
		ut.TraceDebug('+ вз€то значение "' + statName+'"('+paramIndex+') :: ' + res[paramIndex]);
	...
	static function changeStatValue(who, statName:String, indexMult, newValue, paramNumber):Object{
		...
		if (newValue != undefined){
			ut.TraceDebug('+ изменЄн стат '+who.stats[statIndex].value[paramNumber] + " -> " + newValue + '    ' + who.name+':'+  who.stats[statIndex].name + ((indexMult != undefined)? (' (' +indexMult+ ')'): ('')));
			who.stats[statIndex].value[paramNumber] = newValue;
			ut.Trace(who.name + ':'+statName + ' м€не€етс€. '+ "“еперь \"" +statName + "\" = "+ who.stats[statIndex].value);
		}
	...
	static function getStatValue(who, statName:String, indexMult):Array{
		ut.TraceDebug('? ¬«я“№ значение стата "' + statName + '" у ' + who.name);
		if (statExistWithName(who, statName))
			return (statWithName(who, statName, indexMult)).value;
		ut.TraceDebug('! в таком случае добавить стат "' + statName + '" с дефолтным значением дл€ ' + who.name);
		A(who, statName);
		// recursive! kidding
